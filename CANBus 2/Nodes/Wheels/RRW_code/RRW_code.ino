...
void readIncomingMessages(){
    if (mcp2515.readMessage(&canMsg) == MCP2515::ERROR_OK) {
        if(canMsg.can_id == INDIV_WHEEL_THROTTLES_MSG_ID){ //Individual Wheel Throttles
            throttle = canMsg.data[3];
            if(canMsg.data[7] == 1){ //just to ensure reverse doesn't go haywire
                throttleRev = 1;
            } else{
                throttleRev = 0;
            }
...
void setup() {
    //status message
    canStatusMsg.can_id  = RRW_STATUS_MSG_ID;
    canStatusMsg.can_dlc = 1;
    canStatusMsg.data[0] = errorState;

    //wheel Message
    wheelSpeedMsg.can_id  = RR_SPEED_MSG_ID;
